...
const ManageQuizzes = () => {
  const { quizzes, subjects, teachers, attempts, publishQuiz, deleteQuiz } = useQuiz();
  ...
  const rows = useMemo(
    () =>
      quizzes.map((quiz) => {
        ...
        const teacher = teachers.find((candidate) => Number(candidate.id) === Number(quiz.teacherId));
        return {
          ...quiz,
    ...
    [attempts, quizzes, subjects, teachers],
  );

  const visibleRows = rows.filter((quiz) => statusFilter === "all" || quiz.status === statusFilter);
  const { searchTerm, setSearchTerm, filteredItems } = useSearch(visibleRows, ["title", "subjectName", "subjectCode", "teacherName", "difficulty"]);
  ...
  return (
    <div className="fade-in">
      <PageHeader
        ...
        description="View every quiz across teachers, subjects, publishing status, and student activity."
      />
      ...
      <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto]">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search quizzes by title, subject, teacher, or difficulty..."
      ...
      {filteredItems.length === 0 ? (
        <EmptyState icon="file-text" title="No quizzes found" description="Quizzes created by teachers will appear here." />
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/60">
                  ...
                  <th className="hidden px-4 py-3 text-left font-medium text-gray-500 lg:table-cell">Teacher</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-500">Status</th>
                  <th className="hidden px-4 py-3 text-left font-medium text-gray-500 md:table-cell">Activity</th>
              ...
              <tbody>
                {filteredItems.map((quiz) => (
                  <tr key={quiz.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60">
                    ...
                    <td className="hidden px-4 py-3 text-gray-500 lg:table-cell">{quiz.teacherName}</td>
                    <td className="px-4 py-3"><StatusBadge status={quiz.status} /></td>
                    <td className="hidden px-4 py-3 text-gray-500 md:table-cell">{quiz.attempts} attempts</td>
                    <td className="px-4 py-3">
